...
class SCSGatewayService:
    ...
    def add_user(self,request):
        ...
        res = self.user_rpc.add_user(data['username'],data['password'])
        if res['status'] == 'error':
            return Response(str(res),status=HTTPStatus.BAD_REQUEST,headers={'Content-Type':'application/json'})
        else:
            return Response(str(res),status=HTTPStatus.ok,headers={'Content-Type':'application/json'})
